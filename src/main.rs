...
fn user_input() {
    //STANDARD INPUT FROM CLI
        println!("Enter Value: ");
        let mut input = String::new(); 
        io::stdin().read_line(&mut input).expect("failed to read line");  //this command takes user input and save it to our input string.

        //stdin() is a method frome the io module that allows us to read data from
        //the command line

        //read_line(&mut input) passes a mutable reference to the input string so that we 
        //can change the output string.  

        //read_line returns a Result object. the expect method allows us to 
        //check for a valid result from our command line and handles any errors.
        //without this you will get a compiler warning 

        println!("The Value you entered was {}", input);
